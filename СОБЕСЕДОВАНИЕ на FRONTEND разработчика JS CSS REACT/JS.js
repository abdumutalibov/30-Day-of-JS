...
11
// Что такое Promise ?
// Promise -- это специальный объект, предназначенный для работы с ассинчронным 
// кодам и который содержит свое состаяние. Вначале pending(<<ожидание>>), 
...
// Существует специалный синтаксис для работы с промисами, который называется <<async / await>>.
//  Функция обозначенная как async всегда вернет промис
...
18
// 1
// e.preventDefault() и e.stopPropagination() для чего нужны ?
...
try {
    ...
    // Код в этом блоке выполнится, если в 
    // блок try произошла ошибка
} finally {
